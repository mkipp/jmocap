 ...
 * @date 29.06.13
 * 
 * controlls three FacingAngle objects according to two Figures.
 * once they are set the two Figures can not be changed again, but their shoulder bones can!
 * 
 */
...
//        _facingAngleMiddle.setAngleActive(new Vector3d(positionB.x-vPositionMiddle.x, 0,positionB.z-vPositionMiddle.z));
        ...
        if(_angleA > RADIANT90 | _angleB > RADIANT90)_facingAngleMiddle.setInvisible();
        else _facingAngleMiddle.setVisible();
    }
    ...
    public void setHeightA(double h){
        if (Double.isInfinite(h)==false & Double.isNaN(h)== false)_heightA = h;
    }
    
    public void setHeightB(double h){
        if (Double.isInfinite(h)==false & Double.isNaN(h)== false)_heightB = h;
    }
    
    public void setHeightMiddle(double h){
        if (Double.isInfinite(h)==false & Double.isNaN(h)== false)_heightMiddle = h;
    }
