...
public class McNeillGridLogic {
	...
	public void coordinateComparesion(float x, float y, int rightOrLeft) {
		...
		if(-ccw < x && x < ccw) {
			if(-cch < y && y < cch) {
				mng.activateSegment(Segment.CenterCenter, color);
			}
		}
		...
		if(-uw < x && x < uw) {
			if((cch+cw) < y && y < (cch+cw+uh)) {
				mng.activateSegment(Segment.Up, color);
			}
		}
		...
		if(-dw < x && x < dw) {
			if(-(cch+cw) > y && y > -(cch+cw+dh) ) {
				mng.activateSegment(Segment.Down, color);
			}
		}
		...
		if(-(ccw+cw+lw) < x && x < -(ccw+cw)) {
			if(-lh < y && y < lh) {
				mng.activateSegment(Segment.Left, color);
			}
		}
		...
		if((ccw+cw) < x && x < (ccw+cw+rw)) {
			if(-rh < y && y < rh) {
				mng.activateSegment(Segment.Right, color);
			}
		}
		...
		if(ccw < x && x < (ccw+cw+rw)) {
			if((cch+cw) < y && y < (cch+cw+uh)) {
				mng.activateSegment(Segment.UpRight, color);
			}
			else if(-(cch+cw+uh) < y && y < -(cch+cw) ) {
				mng.activateSegment(Segment.DownRight, color);
			}
		}
		if((ccw+cw) < x && x < (ccw+cw+rw)) {
			if(cch < y && y < (cch+cw)) {
		...
		if(-(ccw+cw+lw) < x && x < -ccw) {
			if((cch+cw) < y && y < (cch+cw+uh)) {
				mng.activateSegment(Segment.UpLeft, color);
			}
			else if(-(cch+cw+uh) < y && y < -(ccw+cw)) {
				mng.activateSegment(Segment.DownLeft, color);
			}
		}
		if(-(ccw+cw+lw) < x && x < -(ccw+cw)) {
			if(cch < y && y < (cch+cw)) {
		...
		if(-(ccw+cw) < x && x < (ccw+cw)) {
			if(cch < y && y < (cch+cw)) {
				mng.activateSegment(Segment.Center, color);
			}
			else if(-(cch+cw) < y && y < -cch) {
				mng.activateSegment(Segment.Center, color);
			}
